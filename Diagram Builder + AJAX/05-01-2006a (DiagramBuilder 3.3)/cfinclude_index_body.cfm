			...
			function drawWaferToScale() {
				...
				function displayRecords(_ri, _dict, _rowCntName) {
					...
					//	if (num == 0) _alert('_dict = [' + _dict + ']');
						...
						//	_alert('kPairAR[' + i + '] = [' + kPairAR[i] + ']' + ', datum = [' + datum + ']');
							if (!!datum) {
				...
				if ( (!!oChannelsSel) && ((sel = oChannelsSel.selectedIndex) > -1) && (global_qData.length > 1) && (!!oDiagramFrame) ) {
					...
					D.SetBorder(-10000, 10000, -10000, 10000);
					D.SetGridColor("##FFFFFF", "##EEEEEE");
					D.Draw("##80FF80","##0000FF",true,"Click on me !","parent.F1DiagramClick()");
					...
					if (!!ORGIN_DIE_X) {
						ORGIN_DIE_X = parseFloat(ORGIN_DIE_X);
					}
					var ORGIN_DIE_Y = dictMain.getValueFor('ORGIN_DIE_Y');
					if (!!ORGIN_DIE_Y) {
						ORGIN_DIE_Y = parseFloat(ORGIN_DIE_Y);
					}
					var DIE_PITCH_X = dictMain.getValueFor('DIE_PITCH_X');
					if (!!DIE_PITCH_X) {
						DIE_PITCH_X = parseFloat(DIE_PITCH_X);
					}
					var DIE_PITCH_Y = dictMain.getValueFor('DIE_PITCH_Y');
					if (!!DIE_PITCH_Y) {
						DIE_PITCH_Y = parseFloat(DIE_PITCH_Y);
					}
					drawCircle(D, -10000, -10000, 10000 * 2, 10000 * 2, _asColorHex(0x000000));
					
					var i = -1;
					for (i = 0; i < _data.length; i++) {
						_data[i].OriginX = _data[i].OriginX / 10;
						_data[i].OriginY = _data[i].OriginY / 10;
						new Pixel(D.ScreenX(_data[i].OriginX), D.ScreenY(_data[i].OriginY), "##" + _asColorHex(0x0000ff));
						new Box(_data[i].OriginX, _data[i].OriginY + DIE_PITCH_Y, _data[i].OriginX + DIE_PITCH_X, _data[i].OriginY, "##FF0000", "", "##000000", 1, "##000000");
					}
					// +++
