					...
					if (!!qData) {
						...
						if (!!oO) {
							...
							if (!bool_isAnyErrorRecords) {
								...
								_html += '<br><br>';
								_html += '<span class="normalSmallStatusBoldClass">Data Channel:</span>&nbsp;<select id="selection_channels" class="codeSmallerNoBorder" title="Choose a Metrics Channel to Draw the Wafer..." onchange="global_tidChannel = setInterval(\'disabledChannelSelection()\', 250); return false;">';
							...
							//	_html += '<button id="btn_drawWaferToScale" class="buttonClass" onclick="this.disabled = true; var oSel = $(\'selection_channels\'); if (!!oSel) { oSel.disabled = true; }; drawWaferToScale(); return false;">[Draw Wafer to Scale]</button>';
