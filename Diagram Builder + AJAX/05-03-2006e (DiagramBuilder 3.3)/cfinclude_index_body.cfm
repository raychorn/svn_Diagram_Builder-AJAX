<cfoutput>
	<body onload="window_onload()" onunload="window_onUnload()">
		...
		<!--
				...
				//	D.SetGridColor("##FFFFFF", "##EEEEEE");
					...
					for (i = 0; i < _data.length; i++) {
						...
						if (bool_showMeasurePoints) {
							_data[i].ImageX = Math.ceil(_data[i].ImageX * _diagramScaleFactor);
							_data[i].ImageY = Math.ceil(_data[i].ImageY * _diagramScaleFactor);
							global_titleImagePts[iPt] = 'imageX/imageY = (' + _data[i].ImageX + ',' + _data[i].ImageY + ') ' + ', Metrics Value = (' + _data[i].value + ') '; 
						//	_alert('_data[' + i + '].ImageX = [' + _data[i].ImageX + ']' + ', _data[' + i + '].ImageY = [' + _data[i].ImageY + ']');
							new Box(D.ScreenX(Math.ceil(_data[i].ImageX)), D.ScreenY(Math.ceil(_data[i].ImageY)), D.ScreenX(Math.ceil(_data[i].ImageX) + 40), D.ScreenY(Math.ceil(_data[i].ImageY) + 90), "##" + _asColorHex(0xff0000), "", "##000000", 1, "##" + _asColorHex(0x000000), global_titleImagePts[iPt], "alert(parent.global_titleImagePts[" + iPt + "])");
						}
						// +++
						iPt++;
					}
			...
			var global_tidChannel = -1;
						
			function disabledChannelSelection() {
				var oSel = $('selection_channels');
				var oFrame = $('iframe_diagramMetrics');
				if (global_tidChannel != -1) {
					clearInterval(global_tidChannel);
					global_tidChannel = -1;
				}
				if ( (!!oSel) && (!!oFrame) ) {
					oFrame.style.display = const_none_style;
					drawWaferToScale();
				}
			}
					...
					if (!!qData) {
						...
						if (!!oO) {
							...
							if (!bool_isAnyErrorRecords) {
								...
								_html += '<br><br>';
								_html += '<span class="normalSmallStatusBoldClass">Data Channel:</span>&nbsp;<select id="selection_channels" class="textEntryClass" title="Choose a Metrics Channel to Draw the Wafer..." onchange="global_tidChannel = setInterval(\'disabledChannelSelection()\', 250); return false;">';
								...
								_html += '<select id="selection_notch" class="textEntryClass">';
								...
								_html += '<select id="selection_measurePoints" class="textEntryClass">';
