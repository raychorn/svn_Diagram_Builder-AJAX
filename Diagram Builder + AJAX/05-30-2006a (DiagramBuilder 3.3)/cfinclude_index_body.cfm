...
//	_alert(qObj);
			...
			function drawWaferToScale(j) {
				var oWaferAnalObj = ((j < global_arWaferAnalysisObjects.length) ? global_arWaferAnalysisObjects[j] : null);
				...
				if ( (oWaferAnalObj == null) && (!confirm('The argument j (' + j + ') is invalid as passed into the function known as drawWaferToScale().')) ) {
					return;
				}
				...
				if ( (oWaferAnalObj.qData.length > 1) && (!!oDiagramFrame) ) {
					try {
						try {
							_DiagramTarget = window.frames['F_diagramMetrics' + j];
						...
						for (i = 0; i < oWaferAnalObj._data.length; i++) {
							...
							if (_CurrOriginXY != (_OriginX + '.' + _OriginY)) {
								...
								new Box(theLeft, theTop, theRight, theBottom, "##" + cVal, "", "##000000", 1, "##" + _asColorHex(0x000000), oWaferAnalObj.title[ii], "alert(parent.global_arWaferAnalysisObjects[" + j + "].title[" + ii + "])");
						...
						if (bool_showMeasurePoints) {
							for (i = 0; i < imagePtsStack.length; i++) {
								...
								new Box(_ImageX, _ImageY, _ImageX + 4, _ImageY + 8, "##" + _asColorHex(0xff0000), "", "##000000", 1, "##" + _asColorHex(0x000000), oWaferAnalObj.titleImagePts[i], "alert(parent.global_arWaferAnalysisObjects[" + j + "].titleImagePts[" + i + "])");
							}
						}
...
//	_alert('argsDict = [' + argsDict + ']');
						...
						if (!!oO) {
							...
							//	global_waferStats.resetStats();
								...
								for (var jk = 0; jk < global_arWaferAnalysisObjects.length; jk++) {
									_html += '<iframe id="iframe_diagramMetrics" frameborder="0" scrolling="Auto" width=440 height=250 name="F_diagramMetrics' + jk + '" style="display: none;"></iframe>';
								}
							} else {
								if (bool_isHTMLErrorRecords) {
									_alertHTML(s_explainError);
