	...
	<body onload="window_onload()" onunload="window_onUnload()">
		...
		<!--
			...
			function drawWaferToScale() {
				...
				//	_alert('global_title[' + ii + '] = [' + global_title[ii] + ']');
				...
				//	_alert('dieBoxes.length = [' + dieBoxes.length + ']');
					
					if (0) {
						var dieBoxesSizes = [];
						var dieBoxesWidthDict = DictObj.get$();
						var dieBoxesHeightDict = DictObj.get$();
						var aVal = -1;
						for (i = 0; i < dieBoxes.length; i++) {
							dieBoxesSizes.push(dieBoxes[i].width + '|' + dieBoxes[i].height);
							aVal = dieBoxesWidthDict.getValueFor(dieBoxes[i].width);
							if (aVal == null) {
								dieBoxesWidthDict.push(dieBoxes[i].width.toString(), '1');
							} else {
								dieBoxesWidthDict.put(dieBoxes[i].width.toString(), (parseInt(aVal) + 1).toString());
							}
							aVal = dieBoxesHeightDict.getValueFor(dieBoxes[i].height);
							if (aVal == null) {
								dieBoxesHeightDict.push(dieBoxes[i].height.toString(), '1');
							} else {
								dieBoxesHeightDict.put(dieBoxes[i].height.toString(), (parseInt(aVal) + 1).toString());
							}
						}
					//	_alert('dieBoxesWidthDict = [' + dieBoxesWidthDict + ']' + ', keyForLargestValue = [' + dieBoxesWidthDict.keyForLargestValue() + ']');
					//	_alert('dieBoxesHeightDict = [' + dieBoxesHeightDict + ']' + ', keyForLargestValue = [' + dieBoxesHeightDict.keyForLargestValue() + ']');
					//	_alert('dieBoxesSizes = [' + dieBoxesSizes.toString().split(',').join('\n') + ']');
						
						var uWidth = dieBoxesWidthDict.keyForLargestValue();
						var uHeight = dieBoxesHeightDict.keyForLargestValue();
						var boxObj = -1;
						for (i = 0; i < dieBoxes.length; i++) {
							boxObj = $(dieBoxes[i].ID, _DiagramTarget);
						//	_alert('dieBoxes[' + i + '].ID = [' + dieBoxes[i].ID + ']' + ', boxObj = [' + boxObj + ']');
							if (!!boxObj) {
								boxObj.style.width = uWidth;
								boxObj.style.height = uHeight;
							}
						}
					}
			...
			//	_alert('\nminDieX = [' + minDieX + ']' + ', maxDieX = [' + maxDieX + ']');
			//	_alert('minDieY = [' + minDieY + ']' + ', maxDieY = [' + maxDieY + ']' + '\n');

			//	_alert('minImageX = [' + minImageX + ']' + ', maxImageX = [' + maxImageX + ']');
			//	_alert('minImageY = [' + minImageY + ']' + ', maxImageY = [' + maxImageY + ']' + '\n');

			//	_alert('minOriginX = [' + minOriginX + ']' + ', maxOriginX = [' + maxOriginX + ']');
			//	_alert('minOriginY = [' + minOriginY + ']' + ', maxOriginY = [' + maxOriginY + ']' + '\n');

			//	_alert('minVal = [' + minVal + ']' + ', maxVal = [' + maxVal + ']' + '\n');

			//	_alert('_data.length = [' + _data.length + ']' + '\n');
			}
