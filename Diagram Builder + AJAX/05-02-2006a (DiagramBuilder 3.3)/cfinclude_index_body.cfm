		...
		<!--
			...
			function drawWaferToScale() {
				...
				//	D.SetGridColor("##FFFFFF", "##EEEEEE");
					...
					if (!!ORGIN_DIE_X) {
						ORGIN_DIE_X = parseFloat(ORGIN_DIE_X) * _diagramScaleFactor;
					}
					var ORGIN_DIE_Y = dictMain.getValueFor('ORGIN_DIE_Y');
					if (!!ORGIN_DIE_Y) {
						ORGIN_DIE_Y = parseFloat(ORGIN_DIE_Y) * _diagramScaleFactor;
					}
					var DIE_PITCH_X = dictMain.getValueFor('DIE_PITCH_X');
					if (!!DIE_PITCH_X) {
						DIE_PITCH_X = parseFloat(DIE_PITCH_X) * _diagramScaleFactor;
					}
					var DIE_PITCH_Y = dictMain.getValueFor('DIE_PITCH_Y');
					if (!!DIE_PITCH_Y) {
						DIE_PITCH_Y = parseFloat(DIE_PITCH_Y) * _diagramScaleFactor;
					}
					...
					for (i = 0; i < _data.length; i++) {
						...
						_data[i].OriginX = _data[i].OriginX * _diagramScaleFactor;
						_data[i].OriginY = _data[i].OriginY * _diagramScaleFactor;
						if ((iPt % 2) == 0) {
							_tVal = (tVal / 2);
							_cVal = Math.ceil((_tVal - minVal) / spreadDist) - 1;
							cVal = colorMap[_cVal];
							ii = Math.ceil(iPt / 2);
							global_title[ii] = 'dieX/dieY = (' + _data[i].DieX + ',' + _data[i].DieY + ') ' + _dataPts + ', die total = (' + tVal + ')' + ', die mean = (' + _tVal + ')' + ', Color Index = (' + _cVal + ')';
							new Box(D.ScreenX(Math.ceil(_data[i].OriginX)), D.ScreenY(Math.ceil(_data[i].OriginY)), D.ScreenX(Math.ceil(_data[i].OriginX + DIE_PITCH_X)), D.ScreenY(Math.ceil(_data[i].OriginY + DIE_PITCH_Y)), "##" + cVal, "", "##000000", 1, "##" + _asColorHex(0x000000), global_title[ii], "alert(parent.global_title[" + ii + "])");
							tVal = 0;
							_dataPts = '';
						}
						iPt++;
