		...
		switch (Request.qryStruct.cfm) {
			case 'getWaferDataFromDb':
				if ( (IsDefined("Request.qryStruct.namedArgs.waferName")) AND (IsDefined("Request.qryStruct.namedArgs.channelName")) ) {
					_sql_statement = "SELECT DISTINCT Wafers.id, WaferMetrics.waferID, Wafers.DIE_PITCH_X, Wafers.DIE_PITCH_Y, Wafers.NUMBER_OF_DIES, Wafers.ORGIN_DIE_X, Wafers.ORGIN_DIE_Y, Wafers.POINTS_PER_DIE FROM Wafers INNER JOIN WaferMetrics ON Wafers.id = WaferMetrics.waferID WHERE (Wafers.waferName LIKE '#Right(Request.qryStruct.namedArgs.waferName, 900)#%')";
					Request.commonCode.safely_execSQL('Request.qGetWaferDataFromDb', Request.DSN, _sql_statement);
					if (NOT Request.dbError) {
						Request.commonCode.registerQueryFromAJAX(Request.qGetWaferDataFromDb); // this function is used to tell the AJAX system what Query(s) you wish to communicate back to JavaScript - you can register as many Query Objects as you wish... he CF Var named Request.qryData is used to hold an array of Query Objects...

						_sql_statement = "SELECT id, dieNum, ptNum, dieOriginX, dieOriginY, die_X, die_Y, imageX, imageY, Xrel, Yrel, -1 as '#Request.qryStruct.namedArgs.channelName#' FROM WaferData ORDER BY dieNum, ptNum";
						Request.commonCode.safely_execSQL('Request.qGetWaferLayoutFromDb', Request.DSN, _sql_statement);
						if (NOT Request.dbError) {
							_sql_statement = "SELECT id, waferID, rowNum, #Request.qryStruct.namedArgs.channelName# FROM WaferMetrics WHERE (waferID = #Request.qGetWaferDataFromDb.waferID#) ORDER BY rowNum";
							Request.commonCode.safely_execSQL('Request.qGetWaferMetricsFromDb', Request.DSN, _sql_statement);
							if (NOT Request.dbError) {
								for (i = 1; i lte Request.qGetWaferMetricsFromDb.recordCount; i = i + 1) {
									if (i lte Request.qGetWaferLayoutFromDb.recordCount) {
										Request.qGetWaferLayoutFromDb[Request.qryStruct.namedArgs.channelName][i] = Request.qGetWaferMetricsFromDb[Request.qryStruct.namedArgs.channelName][i];
									}
								}
								Request.commonCode.registerQueryFromAJAX(Request.qGetWaferLayoutFromDb); // this function is used to tell the AJAX system what Query(s) you wish to communicate back to JavaScript - you can register as many Query Objects as you wish... he CF Var named Request.qryData is used to hold an array of Query Objects...
							} else {
				...
				if (IsDefined("Request.qryStruct.namedArgs.metricsFileName")) {
					if (FileExists(Request.qryStruct.namedArgs.metricsFileName)) {
						...
						try {
							...
							writeOutput('<b style="font-size: 12px">A. (metricsStruct / metricsDataAR) bool_isCachedMetricsValid = [#bool_isCachedMetricsValid#]</b><br>');
						} catch (Any e) {
							writeOutput(Request.commonCode.cf_dump(e, '(metricsStruct / metricsDataAR) isCachedMetricsValid Test Failed !', false));
						}
						writeOutput('<b style="font-size: 12px">B. (metricsStruct / metricsDataAR) bool_isCachedMetricsValid = [#bool_isCachedMetricsValid#]</b><br>');
						if (bool_isCachedMetricsValid) {
							writeOutput('<b style="font-size: 12px">A. (metricsStruct / metricsDataAR) Fetching cached Metrics data !</b><br>');
							...
							try {
								...
								writeOutput('<b style="font-size: 12px">1. (metricsStruct) Creating Cached Metrics Struct !</b><br>');
								Application.cachedMetrics[Request.qryStruct.namedArgs.metricsFileName] = StructNew();
							}
							writeOutput('<b style="font-size: 12px">1. (metricsStruct) Saving Cached Metrics Struct !</b><br>');
...
// +++
						if (NOT Request.fileError) {
							...
							try {
								...
								writeOutput('<b style="font-size: 12px">A. (qObj) bool_isCachedMetricsValid = [#bool_isCachedMetricsValid#]</b><br>');
							} catch (Any e) {
								writeOutput(Request.commonCode.cf_dump(e, '(qObj) isCachedMetricsValid Test Failed !', false));
							}
							writeOutput('<b style="font-size: 12px">B. (qObj) bool_isCachedMetricsValid = [#bool_isCachedMetricsValid#]</b><br>');
							if (bool_isCachedMetricsValid) {
								writeOutput('<b style="font-size: 12px">A. (qObj) Fetching cached Metrics data !</b><br>');
								...
								try {
									...
									writeOutput('<b style="font-size: 12px">1. Creating Cached Metrics Struct !</b><br>');
									Application.cachedMetrics[Request.qryStruct.namedArgs.metricsFileName] = StructNew();
								}
							...
							try {
								...
								writeOutput('<b style="font-size: 12px">A. (QOBJ2) bool_isCachedMetricsValid = [#bool_isCachedMetricsValid#]</b><br>');
							} catch (Any e) {
								writeOutput(Request.commonCode.cf_dump(e, '(QOBJ2) isCachedMetricsValid Test Failed !', false));
							}
							writeOutput('<b style="font-size: 12px">B. (QOBJ2) bool_isCachedMetricsValid = [#bool_isCachedMetricsValid#]</b><br>');
							if (bool_isCachedMetricsValid) {
								writeOutput('<b style="font-size: 12px">A. (QOBJ2) Fetching cached Metrics data !</b><br>');
								...
								try {
									...
									writeOutput('<b style="font-size: 12px">2. Creating Cached Metrics Struct !</b><br>');
									Application.cachedMetrics[Request.qryStruct.namedArgs.metricsFileName] = StructNew();
								}
