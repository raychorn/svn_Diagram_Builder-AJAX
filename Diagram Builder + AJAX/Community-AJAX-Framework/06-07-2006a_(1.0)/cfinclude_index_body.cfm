		...
		function handleGetRecentArticles(qObj) {
			...
			function searchForRecentArticles(_ri, _dict, _rowCntName) {
				...
				if ( (!!_ID) && (!!_TITLE) ) {
					...
					html += '&nbsp;&nbsp;<small><NOBR><a href="' + _URL + '">' + _TITLE + '</a></NOBR></small>'; // ((_TITLE.length < 80) ? _TITLE : _TITLE.substring(0,80) + '...')
					html += '</td>';
					html += '</tr>';
					articleNum++;
				}
...
//	_alert('_ALIAS = [' + _ALIAS + ']' + ', _ALLOWCOMMENTS = [' + _ALLOWCOMMENTS + ']' + ', _BODY = [' + _BODY + ']' + ', _CATEGORYIDS = [' + _CATEGORYIDS + ']' + ', _CATEGORYNAMES = [' + _CATEGORYNAMES + ']' + ', _COMMENTCOUNT = [' + _COMMENTCOUNT + ']' + ', _ENCLOSURE = [' + _ENCLOSURE + ']' + ', _FILESIZE = [' + _FILESIZE + ']' + ', _ID = [' + _ID + ']' + ', _MIMETYPE = [' + _MIMETYPE + ']' + ', _MOREBODY = [' + _MOREBODY + ']' + ', _POSTED = [' + _POSTED + ']' + ', _TITLE = [' + _TITLE + ']' + ', _TRACKBACKCOUNT = [' + _TRACKBACKCOUNT + ']' + ', _USERNAME = [' + _USERNAME + ']' + ', _POSTEDLONG = [' + _POSTEDLONG + ']');
//	_alert('(!!_ALIAS) = [' + (!!_ALIAS) + ']' + ', (!!_ALLOWCOMMENTS) = [' + (!!_ALLOWCOMMENTS) + ']' + ', (!!_BODY) = [' + (!!_BODY) + ']' + ', (!!_CATEGORYIDS) = [' + (!!_CATEGORYIDS) + ']' + ', (!!_CATEGORYNAMES) = [' + (!!_CATEGORYNAMES) + ']' + ', (!!_COMMENTCOUNT) = [' + (!!_COMMENTCOUNT) + ']' + ', (!!_ENCLOSURE) = [' + (!!_ENCLOSURE) + ']' + ', (!!_FILESIZE) = [' + (!!_FILESIZE) + ']' + ', (!!_ID) = [' + (!!_ID) + ']' + ', (!!_MIMETYPE) = [' + (!!_MIMETYPE) + ']' + ', (!!_MOREBODY) = [' + (!!_MOREBODY) + ']' + ', (!!_POSTED) = [' + (!!_POSTED) + ']' + ', (!!_TITLE) = [' + (!!_TITLE) + ']' + ', (!!_TRACKBACKCOUNT) = [' + (!!_TRACKBACKCOUNT) + ']' + ', (!!_USERNAME) = [' + (!!_USERNAME) + ']' + ', (!!_POSTEDLONG) = [' + (!!_POSTEDLONG) + ']');
...
//	_alert('(!!qData2) = [' + (!!qData2) + ']');
				if ( (!!qData1) && (!!qData2) ) {
					...
					if (!!oParms) {
						oParms.iterateRecObjs(searchForArgRecs);
						qData1.iterateRecObjs(anyErrorRecords);
//	_alert('bool_isAnyErrorRecords = [' + bool_isAnyErrorRecords + ']');
						if (!bool_isAnyErrorRecords) {
							argsDict.intoNamedArgs();
							
							qData2.iterateRecObjs(searchForBlogSettings);
		...
		//		_alert('blogSettingsDict = [' + blogSettingsDict + ']');
						} else {
							if (bool_isHTMLErrorRecords) {
								_alertHTML(s_explainError);
							} else {
		...
		<tr>
			<td valign="top">
				<table width="100%" cellpadding="-1" cellspacing="-1">
					...
					<tr>
						<td id="td_articleContent" width="70%" valign="top" style="border-right: thin dotted Silver;">
							<div id="div_articleContent" style="overflow: auto;"></div>
						</td>
						<td id="td_podContent2" width="25%" valign="top" style="display: none; border-left: thin dotted Silver;">
							<div id="div_podContent2"></div>
						</td>
						<td id="td_podContent" width="30%" valign="top" style="border-left: thin dotted Silver;">
