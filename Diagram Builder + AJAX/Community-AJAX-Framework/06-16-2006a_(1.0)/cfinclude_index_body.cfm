...
<cfif 0>
	...
	<!--
		...
		function searchPod() {
			...
			_html += '</p>';
			return podLayoutHTML(_html, ((jsBool_isCurrentBlogDefault) ? js_searchTitle : 'SEARCH MY DATABASE'), 'searchPod', '&td=' + 'align="center"'.URLEncode() + '&td=' + ('bgColor="' + js_blogBgColor + '"').URLEncode());
		}
		...
		//		_alert('jsBool_isCurrentBlogDefault = [' + jsBool_isCurrentBlogDefault + ']' + ', jsCurrentBlogName = [' + jsCurrentBlogName + ']');
				html = logoPod() + ((jsBool_isCurrentBlogDefault) ? productsPod() : defaultBlogPod()) + searchPod();
