<cfoutput>
	<cfscript>
		...
		bool_sessionLoggedin = ( (IsDefined("session.persistData.loggedin")) AND (session.persistData.loggedin) );
...
<cfif 0>
	...
	<!--
		...
		var registerFormAlreadyPopulated = false;
		...
		function repositionArticleContent() {
				...
				if (!!qData1) {
					...
					if (!!oParms) {
						...
						if (!bool_isAnyErrorRecords) {
							...
							if (!!_STATUSMSG) alert(_STATUSMSG);
						} else {
