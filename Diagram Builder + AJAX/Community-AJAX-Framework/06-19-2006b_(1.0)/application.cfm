...
<cfif (FileExists(ExpandPath("AJAXApp/initialize.cfm")))>
	<cfinclude template="AJAXApp/initialize.cfm">
</cfif>

<cfset Request.bool_sessionLoggedin = ( (IsDefined("session.persistData.loggedin")) AND (session.persistData.loggedin) AND (IsDefined("Session.persistdata.QAUTHUSER")) AND (IsQuery(Session.persistdata.QAUTHUSER)) )>

<cfset makeNewRuntimeLicense = (Request.commonCode.isServerLocal()) AND false>

<!--- BEGIN: Read the Runtime License File --->
<!--- To-Do:
	(1). Make data in RuntimeLicense.dat file binary.
	(2). Store checksum in data stream.
	(3). Winbatch Installer - 
		(3.1). Get the Machine ID - code written.
		(3.2). Determine if Machine ID is in Database. Using HTTP interface.
		(3.3). Create a Runtime License and download it to Installer using FTP interface.
 --->
<cfif (NOT makeNewRuntimeLicense)>
	<cfscript>
		aStruct = Request.commonCode.readRuntimeLicenseFile();
	//	writeOutput(Request.commonCode.cf_dump(aStruct, 'aStruct', false));
