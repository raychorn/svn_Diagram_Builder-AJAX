...
// +++							
	...
	//			_alert('oBtn.id = [' + oBtn.id + ']' + ', iLevel = [' + iLevel + ']' + ', oBtn.name = [' + oBtn.name + ']' + ', dName = [' + dName + ']' + ', boolFireButtonAction = [' + boolFireButtonAction + ']');
				if (boolFireButtonAction) {
					oBtn.disabled = true;
...
//	_alert('isLevelZero = [' + isLevelZero + ']' + ', _indentLevel = [' + _indentLevel + ']');
							html += '<table width="100%" cellpadding="-1" cellspacing="-1">';
							qData1.iterateRecObjs(searchForDirData);
							html += '</table>';
		...
		//	_alert('performCalendarPrevMonth(' + qStr + ')');
		//	_alert('dict = [' + dict + ']');
		//	_alert('_dt = [' + _dt + ']');
		...
		//	_alert('performCalendarNextMonth(' + qStr + ')');
		//	_alert('dict = [' + dict + ']');
		//	_alert('_dt = [' + _dt + ']');
			doAJAX_func('getCalendarMetaData', 'handleGetCalendarMetaData(' + oAJAXEngine.js_global_varName + ')', 'month', _dt.getMonth(), 'year', _dt.getFullYear());
		}
			...
			function calendarContent() {
				...
				if ( (!!_ISBIDI) && (!!_LOCALIZEDMONTH) && (!!_LOCALIZEDYEAR) && (!!_LOCALIZEDDAYS) && (!!_FIRSTWEEKPAD) && (!!_DAYCOUNTER) && (!!_DIM) && (!!_ROWCOUNTER) ) {
					...
					if (_ISBIDI) {
						...
						html += '<button type="button" id="btn_calendar_this" class="buttonClass" onclick="return false;">' + '&nbsp;&nbsp;' + _LOCALIZEDMONTH + '&nbsp;' + _LOCALIZEDYEAR + '&nbsp;&nbsp;' + '</button>';
