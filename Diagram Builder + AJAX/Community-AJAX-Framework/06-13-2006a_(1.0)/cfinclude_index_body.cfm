<cfoutput>
	<cfscript>
		...
		if (bool_sessionLoggedin) {
			...
			cf_CodeWaferAnalysis = "window.open('#_urlWaferAnalysis#','samples1','width=' + clientWid$() + ',height=' + clientHt$() + ',resizeable=yes,scrollbars=1')";
		} else {
...
<cfif 0>
	...
	<!--
		...
		function performOpenLoginRegisterWindow(id, hRef) {
			...
			if ( (!!oTD) && (!!oFrame) && (typeof hRef == const_string_symbol) && (!!oBtn) && (!!oBtn2) && (!!oBtn3) ) {
		...
		function performCloseLoginRegisterWindow() {
			...
			var oFrame = _$('iframe_loginRegisterFrame'); 
			if ( (!!oTD) && (!!oFrame) && (!!oBtn) && (!!oBtn2) && (!!oBtn3) && (!!oBtn4) && (!!oBtn5) ) {
		...
		function windowOnscrollCallback(top, left) {
			...
			if ( (!!dObj) && (!!oAnchorPosTop) ) {
			//	dObj.style.top = document.body.scrollTop + ((bool_isScrollLocked) ? (oAnchorPosTop.y - 25) : 0) + 'px';
	...
	//			jsErrorExplainer(e, '111', true);
		...
		var tid_updateBlogStatsDateTime = setInterval("updateBlogStatsDateTime()", 1000);
		var tid_ajax_updateBlogStatsDateTime = setInterval("ajax_updateBlogStatsDateTime()", 1000 * 30);
		...
		function handleSubmitRegisterForm(qObj) {
			...
			function searchForStatusRecs(_ri, _dict, _rowCntName) {
				...
				if ( (aDict == -1) && ( ( (!!_ISVALID) && (!!_LOGGEDIN) ) || (!!_ISFORGOTPASSWORD) || (!!_ISBLOGSUBSCRIBED) || (!!_ACTUALUSERSONLINE) ) ) {
...
//	_alert('aDict = [' + aDict + ']')
							...
							if ( (!!_ISVALID) && (!!_LOGGEDIN) ) {
								...
								if (!!oDiv) {
							//		oDiv.innerHTML = '<span class="textClass">' + _ACTUALUSERSONLINE + '</span>';
		...
		function performViewingArticlesByDay(id) {
			if (!!id) {
				var ar = id.split('_');
				if (ar.length > 3) {
					doAJAX_func('getBlogArticlesByDay', 'handleGetDefaultBlogArticles(' + oAJAXEngine.js_global_varName + ')', 'month', ar[ar.length - 3], 'day', ar[ar.length - 2], 'year', ar[ar.length - 1]);
				}
			}
		}
		...
		function handleGetCalendarMetaData(qObj) {
			...
			function calendarContent() {
				...
				if ( (_ISBIDI != null) && (_LOCALIZEDMONTH != null) && (_LOCALIZEDYEAR != null) && (_LOCALIZEDDAYS != null) && (_FIRSTWEEKPAD != null) && (_DAYCOUNTER != null) && (_DIM != null) && (_ROWCOUNTER != null) && (_DAYLIST != null) && (_FIRSTDAY != null) ) {
					...
					if (_ISBIDI) {
						html += '<button type="button" id="btn_calendar_prev" class="buttonClass" onclick="performCalendarPrevMonth(\'' + aDict.asQueryString(',', true) + '\'); return false;">' + '&nbsp;&nbsp;&lt;&lt;&nbsp;&nbsp;' + '</button>';
						html += '<button type="button" id="btn_calendar_this" class="buttonClass" onclick="performViewingArticlesByMonth(\'' + aDict.asQueryString(',', true) + '\'); return false;">' + '&nbsp;&nbsp;' + _LOCALIZEDMONTH + '&nbsp;' + _LOCALIZEDYEAR + '&nbsp;&nbsp;' + '</button>';
						html += '<button type="button" id="btn_calendar_next" class="buttonClass" onclick="performCalendarNextMonth(\'' + aDict.asQueryString(',', true) + '\'); return false;">' + '&nbsp;&nbsp;&gt;&gt;&nbsp;&nbsp;' + '</button>';
					} else {
						html += '<button type="button" id="btn_calendar_prev" class="buttonClass" onclick="performCalendarPrevMonth(\'' + aDict.asQueryString(',', true) + '\'); return false;">' + '&nbsp;&nbsp;&lt;&lt;&nbsp;&nbsp;' + '</button>';
						html += '<button type="button" id="btn_calendar_this" class="buttonClass" onclick="performViewingArticlesByMonth(\'' + aDict.asQueryString(',', true) + '\'); return false;">' + '&nbsp;&nbsp;' + _LOCALIZEDMONTH + '&nbsp;' + _LOCALIZEDYEAR + '&nbsp;&nbsp;' + '</button>';
						html += '<button type="button" id="btn_calendar_next" class="buttonClass" onclick="performCalendarNextMonth(\'' + aDict.asQueryString(',', true) + '\'); return false;">' + '&nbsp;&nbsp;&gt;&gt;&nbsp;&nbsp;' + '</button>';
		...
		function handleGetDefaultBlogArticles(qObj) {
			...
			function searchForBlogArticles(_ri, _dict, _rowCntName) {
				...
				if ( (_ALIAS != null) && (_ALLOWCOMMENTS != null) && (_BODY != null) && (_CATEGORYIDS != null) && (_CATEGORYNAMES != null) && (_COMMENTCOUNT != null) && (_ENCLOSURE != null) && (_FILESIZE != null) && (_ID != null) && (_MIMETYPE != null) && (_MOREBODY != null) && (_POSTED != null) && (_TITLE != null) && (_TRACKBACKCOUNT != null) && (_USERNAME != null) && (_POSTEDLONG != null) ) {
					...
					for (i = 0; i < ar.length; i++) {
						isCurrentCategory = (ar[i].toUpperCase() == currentCategoryName.toUpperCase());
						html += ((isCurrentCategory) ? '<span class="currentCategoryBoldClass">' : '<a href="" onclick="performViewingArticlesByCategoryName(\'' + ar[i] + '\'); return false;">') + ar[i] + ((isCurrentCategory) ? '</span>' : '</a>') + ((i != (ar.length - 1)) ? ',' : '');
					}
					html += '</div>';
