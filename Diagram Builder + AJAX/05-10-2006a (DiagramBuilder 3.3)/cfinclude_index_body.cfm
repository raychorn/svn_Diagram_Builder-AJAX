...
//	_alert(qObj);
				...
				if (nRecs > 0) { // at present only the first data record is consumed...
					...
					//	_alert('oBtnA = [' + oBtnA + ']' + ', oBtnB = [' + oBtnB + ']');
								if ( (!!oBtnA) && (!!oBtnB) ) {
									...
									oBtnA.disabled = ((dataDict.getValueFor('WAFERFULLNAME') == pName) ? false : true);
									oBtnB.disabled = ((oBtnA.disabled) ? false : true);
					//	_alert('oBtnB.title = [' + oBtnB.title + ']' + ', pName = [' + pName + ']' + '\nqData1 = [' + qData1 + ']');
								} else {
					...
					for (i = 0; i < cols.length; i++) {
						if (_dict.getValueFor('ATTRIBUTES').toUpperCase().indexOf('H') == -1) {
							if (locateArrayItems(dontShowCols, cols[i]) == -1) {
								...
								_html += '<span class="normalStatusBoldClass">' + ((isFolder) ? '<button id="btn_dirDrillInto_DiskBrowserGUI_' + _ri + '" class="buttonClass" onclick="handle_dirDrillInto_DiskBrowserGUI(this); return false;">' : ((isPythonFile) ? '<button id="btn_pythonProcess_DiskBrowserGUI" class="buttonClass" value="' + datum + '" title="\n' + _dict.asQueryString() + '" onclick="this.disabled = true; handle_pythonProcess_DiskBrowserGUI(this); return false;">' : '')) + datum + ((isFolder) ? '</button>' : (isPythonFile ? '</button>' : '')) + ((isPythonFile) ? '&nbsp;&nbsp;<button id="btn_dbProcess_DiskBrowserGUI" disabled class="buttonClass" title="\n' + _dict.asQueryString() + '" onclick="this.disabled = true; displayMetricsBrowserDiagramInterface(); return false;">[Show Wafer from Db]</button>' : '') + '</span>';
								_html += '</td>';
							}
						}
					}
...
// +++
								...
								//	alert('oBtnFaster = [' + oBtnFaster + ']' + ', oBtnSlower = [' + oBtnSlower + ']' + ', oWaferName = [' + oWaferName + ']');
									...
									//	alert('ERROR: Programming Error - Missing btn_pythonProcess_DiskBrowserGUI or btn_faster_waferDisplay.');
									}
								}
