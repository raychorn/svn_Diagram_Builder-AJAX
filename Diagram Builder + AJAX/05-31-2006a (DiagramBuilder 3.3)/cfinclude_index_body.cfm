			...
			function drawWaferToScale(jNum) {
				...
				if ( (oWaferAnalObj.qData.length > 1) && (!!oDiagramFrame) ) {
					try {
						...
						try {
							var D = new Diagram();
							var _dgWidth = (540 / global_arWaferAnalysisObjects.length);
							var _dgHeight = (460 / global_arWaferAnalysisObjects.length);
							if (global_arWaferAnalysisObjects.length == 1) {
								D.SetFrame(80, 10, _dgWidth, _dgHeight);
							} else {
								var baseLeft = 80;
								var baseTop = 10;
								var baseRight = _dgWidth;
								var baseBottom = _dgHeight;

								var frameLeft = baseLeft + (jNum * _dgWidth);
								var frameTop = baseTop + (jNum * _dgHeight);
								var frameRight = baseLeft + (jNum * _dgWidth) + _dgWidth;
								var frameBottom = baseTop + (jNum * _dgHeight) + _dgHeight;

								D.SetFrame(frameLeft, frameTop, frameRight, frameBottom);
					_alert('jNum = [' + jNum + ']' + ', frameLeft = [' + frameLeft + ']' + ', frameTop = [' + frameTop + ']' + ', frameRight = [' + frameRight + ']' + ', frameBottom = [' + frameBottom + ']');
							}
