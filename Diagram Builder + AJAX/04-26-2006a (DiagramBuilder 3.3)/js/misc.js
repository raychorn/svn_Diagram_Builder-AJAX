...
function $(id) {
	var oO = -1;
	if ($cache[id] == null) { oO = _$(id); $cache[id] = oO;} else {oO = $cache[id];}
	return oO;
}
	...
	if ( (!!cObj) && (!!bObj) && (!!tbObj) ) { 
		...
		bObj.value = ((cObj.style.display == const_inline_style) ? '[<<]' : '[>>]'); 
	   	try {
			...
			_alertError(jsErrorExplainer(e, 'handle_ajaxHelper2_onClick()' + ' _global_clientWidth = [' + _global_clientWidth + ']'));
		}
	};
}
