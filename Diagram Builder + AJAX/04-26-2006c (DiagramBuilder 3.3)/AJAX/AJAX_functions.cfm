			...
			case 'processPythonFile':
				if (IsDefined("Request.qryStruct.namedArgs.pythonFileName")) {
					if (FileExists(Request.qryStruct.namedArgs.pythonFileName)) {
						Request.commonCode.cf_file('READ', Request.qryStruct.namedArgs.pythonFileName, Request.pythonContent);
			
						if (NOT Request.fileError) {
							qObj = QueryNew('id, statusMsg');
							QueryAddRow(qObj, 1);
							QuerySetCell(qObj, 'id', qObj.recordCount, qObj.recordCount);
							QuerySetCell(qObj, 'statusMsg', 'There are (#Len(Request.pythonContent)#) bytes.', qObj.recordCount);
