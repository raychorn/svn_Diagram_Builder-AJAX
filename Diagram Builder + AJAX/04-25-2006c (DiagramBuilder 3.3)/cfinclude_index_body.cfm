		...
		<!--
			...
			function handle_graphType_onchange(oObj) {
				if (!!oObj) {
					...
					//	alert('oObj.options[' + sel + '].text = [' + oObj.options[sel].text + ']' + ', oObj.options[' + sel + '].value = [' + oObj.options[sel].value + ']');
					}
				}
			}
			...
			function drawData(graphicalChart) {
				...
				switch (graphicalChart) {
					...
					case 10:
						...
						new Box(570, 180, 640, 220, Color[0], Name[0]+"<br><img src='smile.gif'  width=20 height=20 border=0>", BGColor[0], 2, "##000000", Job[0], "parent.ShowPhoneNum(0)");
