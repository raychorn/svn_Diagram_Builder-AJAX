			...
			function drawWaferToScale() {
				...
				if (global_qData == null) {
					...
					if ( (!!oChannelsSel) && ((sel = oChannelsSel.selectedIndex) > -1) && (global_qData.length > 1) && (!!oDiagramFrame) ) {
						...
						if (_data.length == 0) {
							for (jj = 0; jj < global_qData.length; jj++) {
								global_qData[jj].iterateRecObjs(displayRecords);
							}
						} else {
							for (jj = 0; jj < global_qData.length; jj++) {
								global_qData[jj].iterateRecObjs(displayRecordsFast);
							}
						}
					...
					//	D.SetGridColor("##FFFFFF", "##EEEEEE");
						...
						for (jj = 0; jj < global_qData.length; jj++) {
							global_qData[jj].iterateRecObjs(getMainDict);
						}
