	...
	<cfscript>
		...
		if (bool_RuntimeLicenseStatus) {
			if (FindNoCase(CGI.SERVER_NAME, CGI.HTTP_REFERER) gt 0) {
				if ( (1) OR ( (IsDefined("Request.qryStruct.sessid")) AND (IsValid('UUID', Request.qryStruct.sessid)) AND (IsDefined("Session.persistData.loggedin")) AND (Session.persistData.loggedin) ) ) {
					...
					QueryAddRow(qObj, 1);
					QuerySetCell(qObj, 'id', qObj.recordCount, qObj.recordCount);
					QuerySetCell(qObj, 'errorMsg', '<br><br><br><h5 style="color: red">Use of this AJAX Server is strictly limited to those who have valid User Accounts for this site.</h5>', qObj.recordCount);
