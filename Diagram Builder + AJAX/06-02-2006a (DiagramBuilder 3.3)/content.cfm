...
<cfsavecontent variable="jsCode">function object_destructor(oO){var rV=-1;try{rV=oO.destructor();}catch(e){rV=null;}finally{rV=null;};return rV;}
...
function _alertHTMLError(s){return dispayHTMLSysMessages(s,'ERROR');}
...
var bool_isAnyErrorRecords=false;var bool_isAnyPKErrorRecords=false;var bool_isHTMLErrorRecords=false;function anyErrorRecords(_ri,_dict,_rowCntName){var errorMsg='';var isPKerror='';var isHTML='';try{errorMsg=_dict.getValueFor('ERRORMSG');isPKerror=_dict.getValueFor('ISPKVIOLATION');isHTML=_dict.getValueFor('ISHTML');}catch(e){}finally{}
bool_isAnyErrorRecords=((!!errorMsg)&&(errorMsg.length>0));bool_isAnyPKErrorRecords=((!!isPKerror)&&(isPKerror.length>0));bool_isHTMLErrorRecords=((!!isHTML)&&(isHTML.length>0));s_explainError='';if((bool_isAnyErrorRecords)||(bool_isAnyPKErrorRecords)||(bool_isHTMLErrorRecords)){try{if(bool_isHTMLErrorRecords){s_explainError+=_dict.getValueFor('ERRORMSG');}else{s_explainError+=_dict.getValueFor('ERRORMSG').stripHTML().stripSpacesBy2s().stripCrLfs().stripTabs(' ');}}catch(e){}finally{}
try{if(bool_isHTMLErrorRecords){s_explainError+='\n'+_dict.getValueFor('MOREERRORMSG');}else{s_explainError+='\n'+_dict.getValueFor('MOREERRORMSG').stripHTML().stripSpacesBy2s().stripCrLfs().stripTabs(' ');}}catch(e){}finally{}}}
...
WaferAnalObj=function(id,_waferName,_waferID){this.id=id;this.waferName=_waferName;this.waferID=_waferID;this.waferStats=-1;this.title=-1;this.titleImagePts=-1;this._data=-1;this.qData=-1;};WaferAnalObj.$=[];WaferAnalObj.get$=function(waferName,waferID){var instance=WaferAnalObj.$[WaferAnalObj.$.length];if(instance==null){instance=WaferAnalObj.$[WaferAnalObj.$.length]=new WaferAnalObj(WaferAnalObj.$.length,waferName,waferID);}
return instance;};WaferAnalObj.remove$=function(id){var ret_val=false;if((id>-1)&&(id<WaferAnalObj.$.length)){var instance=WaferAnalObj.$[id];if(!!instance){WaferAnalObj.$[id]=object_destructor(instance);ret_val=(WaferAnalObj.$[id]==null);}}
return ret_val;};WaferAnalObj.remove$s=function(){var ret_val=true;for(var i=0;i<WaferAnalObj.$.length;i++){WaferAnalObj.remove$(i);}
return ret_val;};WaferAnalObj.prototype={id:-1,waferName:-1,waferID:-1,waferStats:-1,title:-1,titleImagePts:-1,_data:-1,qData:-1,toString:function(){var s='id = ('+this.id+') :: ';s+='waferName = ('+this.waferName+')';s+=', waferID = ('+this.waferID+')';s+=', waferStats = ('+this.waferStats+')';s+=', title = ('+this.title+')';s+=', titleImagePts = ('+this.titleImagePts+')';s+=', _data = ('+this._data+')';s+=', qData = ('+this.qData+')';s+='\n';return s;},init:function(){return this;},destructor:function(){return(this.id=WaferAnalObj.$[this.id]=null);}};
</cfsavecontent>

<cfif (FindNoCase(_serverName, CGI.HTTP_REFERER) gt 0)>
	<cftry>
		<cfset fname = GetDirectoryFromPath(GetBaseTemplatePath()) & CreateUUID()>
		<cfset _jsCode = Request.commonCode._obfuscateJSCode(jsCode)>
		<cffile action="WRITE" file="#fname#" output="#_jsCode#" attributes="Hidden" addnewline="No" fixnewline="No">
