					...
					if (FileExists(Request.qryStruct.namedArgs.metricsFileName)) {
						...
						//	qObj = QueryNew('id, debugContent');
							...
							for (i = 1; i lte ArrayLen(metricsDataAR); i = i + 1) {
								...
								chNum = 1;
							Request.commonCode.cf_dump(ar, 'ar', false);
						...
						if (NOT Request.fileError) {
							qObj = QueryNew('id, dieNum, ptNum, die_x, die_y, imagex, imagey, xrel, yrel, channel1, channel2, channel3, channel4, channel5, channel6, channel7, channel8, channel9');
							...
							for (dieNum = 0; dieNum lt num_dies; dieNum = dieNum + 1) {
								for (ptNum = 0; ptNum lt num_points; ptNum = ptNum + 1) {
									...
									QuerySetCell(qObj, 'die_x', metricsStruct['#dieNum#.#ptNum#']['die_x'], qObj.recordCount);
									QuerySetCell(qObj, 'die_y', metricsStruct['#dieNum#.#ptNum#']['die_y'], qObj.recordCount);
									QuerySetCell(qObj, 'imagex', metricsStruct['#dieNum#.#ptNum#']['imagex'], qObj.recordCount);
									QuerySetCell(qObj, 'imagey', metricsStruct['#dieNum#.#ptNum#']['imagey'], qObj.recordCount);
									QuerySetCell(qObj, 'xrel', metricsStruct['#dieNum#.#ptNum#']['xrel'], qObj.recordCount);
									QuerySetCell(qObj, 'yrel', metricsStruct['#dieNum#.#ptNum#']['yrel'], qObj.recordCount);
