		...
		<!--
			...
			function handle_submit_DiskBrowserGUI(btnObj) {
				...
				if ( (!!btnObj) && (!!iObj) ) {
					...
					global_baseFolderName = global_pathName = '';
			...
			function htmlDiskBrowserGUI() {
				...
				_html += '<input id="input_DiskBrowserGUI_driveLetter" type="Text" class="textEntryClass" value="c:\\" title="Enter a Drive Letter or UNC Path for the location of the data files you wish to work with." size="90" maxlength="255">';
				_html += '</td>';
				_html += '</tr>';
				_html += '</table>';
					...
					for (i = 0; i < cols.length; i++) {
						if (_dict.getValueFor('ATTRIBUTES').toUpperCase().indexOf('H') == -1) {
							if (locateArrayItems(dontShowCols, cols[i]) == -1) {
								...
								_html += '<span class="normalStatusBoldClass">' + ((isFolder) ? '<button id="btn_dirDrillInto_DiskBrowserGUI_' + _ri + '" class="buttonClass" onclick="handle_dirDrillInto_DiskBrowserGUI(this); return false;">' : '') + _dict.getValueFor(cols[i]) + ((isFolder) ? '</button>' : '') + '</span>';
								_html += '</td>';
							}
						}
					}
...
//	_alert(qObj);				
		...
		<div id="form_diskBrowser" style="position: absolute; top: 100px; left: 10px; width: 720px; height: 100px; display: none;"></div>
