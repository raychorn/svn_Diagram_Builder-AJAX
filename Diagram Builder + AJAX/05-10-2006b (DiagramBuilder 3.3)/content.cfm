...
aa+=']';}
return aa;},destructor:function(){return(this.id=QObj.$[this.id]=this.s_toString=this.colNames=this.dataRec=null);}};GUIActsObj=function(id){this.id=id;this.stack=[];this.ooStack=[];this.aspectStack=[];this.stylesStack=[];};GUIActsObj.$=[];GUIActsObj.get$=function(){var instance=GUIActsObj.$[GUIActsObj.$.length];if(instance==null){instance=GUIActsObj.$[GUIActsObj.$.length]=new GUIActsObj(GUIActsObj.$.length);}
...
return ret_val;};GUIActsObj.prototype={id:-1,stack:[],ooStack:[],aspectStack:[],stylesStack:[],toString:function(){var s='id = ('+this.id+') :: (';s+='\n';for(var i=0;i<this.stack.length;i++){s+=i+' = ['+this.stack[i]+']'+((i<(this.stack.length-1))?', ':'');}
s+=')';return s;},push:function(id){var aHandle=-1;var oo=$(id);if(!!oo){this.stack.push(id);this.ooStack.push(oo);this.aspectStack.push(DictObj.get$());this.stylesStack.push(DictObj.get$());aHandle=this.stack.length-1;}
...
function showAJAXBegins(sMsg,sImage,imageHeight){var resp='';var frameCode='';var divName=const_cf_html_container_symbol;var cObj=$(divName);if(!!cObj){imageHeight=((!!imageHeight)?imageHeight:'55%');resp='<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN">';resp+='<html><head><title>System Activity Display</title>';resp+='<link rel=stylesheet type="text/css" href="'+document.location.href+'StyleSheet.css">';resp+='</head><body>';resp+=ajaxBegins();resp+='<b>'+((!!sMsg)?sMsg:const_loading_data_message_symbol)+'</b>';resp+='&nbsp;&nbsp;<img height="'+imageHeight+'" src="'+((!!sImage)?sImage:const_jsapi_loading_image)+'" border="0">';resp+=ajaxEnds();resp+='</body></html>';frameCode='<iframe id="iframe_showAJAXBegins" frameborder="0" marginwidth="0" marginheight="0" scrolling="No" style="display: none;"></iframe>';if(cObj.innerHTML.length>0){flushCache$(cObj);}
cObj.innerHTML=frameCode;var frObj=$('iframe_showAJAXBegins');if(!!frObj){frObj.contentWindow.document.writeln(resp);frObj.style.display=const_inline_style;cObj.style.position='absolute';cObj.style.top='0px';cObj.style.left=(clientWid$()-const_jsapi_width_value-1)+'px';cObj.style.height='30px';cObj.style.width=const_jsapi_width_value+'px';frObj.style.top='0px';frObj.style.left='0px';frObj.style.height='30px';frObj.style.width=const_jsapi_width_value+'px';}
