		...
		switch (Request.qryStruct.cfm) {
			case 'getWaferDataFromDb':
				if ( (IsDefined("Request.qryStruct.namedArgs.waferName")) AND (IsDefined("Request.qryStruct.namedArgs.channelName")) ) {
					_sql_statement = "SELECT Wafers.id, WaferMetrics.waferID, Wafers.waferName, Wafers.waferFullName, Wafers.DIE_PITCH_X, Wafers.DIE_PITCH_Y, Wafers.NUMBER_OF_DIES, Wafers.ORGIN_DIE_X, Wafers.ORGIN_DIE_Y, Wafers.POINTS_PER_DIE, WaferMetrics.rowNum, WaferMetrics.#Request.qryStruct.namedArgs.channelName# FROM Wafers INNER JOIN WaferMetrics ON Wafers.id = WaferMetrics.waferID WHERE (Wafers.waferName LIKE '#Right(Request.qryStruct.namedArgs.waferName, 900)#%') ORDER BY WaferMetrics.rowNum";
					...
					if (NOT Request.dbError) {
						...
						_sql_statement = "SELECT id, dieNum, ptNum, dieOriginX, dieOriginY, die_X, die_Y, imageX, imageY, Xrel, Yrel FROM WaferData ORDER BY dieNum, ptNum";
