...
//	_alert(qObj);
			...
			function handle_onchange_select_waferRecords(oSel) {
				...
				if (!!oSel) {
					...
					var oBtn = $('btn_display_WafersRecordsBrowserGUI');
					if (!!oBtn) {
						oBtn.disabled = ((_ids.length > 1) ? false : true);
					}
				}
			...
			function handle_display_WaferRecordsBrowserGUI(oBtn) {
				...
				if ( (!!oBtn) && (!!oSel) ) {
					...
					if (oSel.options.length > 1) {
						bool = confirm('You have selected too many wafers to display using the Display One Wafer method.  The wafer named "' + oSel.options[0].text + '" will be displayed but the others will not. Click the cancel button to adjust your selections before trying again.');
					}
					if (bool) {
						for (i = 0; i < oSel.options.length; i++) {
							if (oSel.options[i].selected) {
